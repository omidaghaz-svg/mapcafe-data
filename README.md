...
| --- | --- | --- | --- | --- | --- |
| رستوران | ۹٬۰۲۶ | ۷٬۷۳۳ | ۱٬۲۳۶ | ۵۷ | ۸٬۳۴۹ |
| کافه | ۴٬۹۸۶ | ۴٬۶۸۵ | ۲۸۹ | ۱۲ | ۴٬۵۴۶ |
| فست‌فود | ۴٬۲۴۰ | ۴٬۰۱۵ | ۱۹۸ | ۲۷ | ۳٬۸۶۸ |
| آبمیوه و بستنی | ۱٬۲۶۵ | ۱٬۱۸۸ | ۶۹ | ۸ | ۱٬۱۶۰ |
| کباب | ۱٬۷۲۳ | ۱٬۵۵۱ | ۱۶۰ | ۱۲ | ۱٬۶۸۹ |
| طباخی | ۵۵۲ | ۵۲۱ | ۲۸ | ۳ | ۵۴۹ |

از این میان ۱۶٬۳۱۱ مکان تک‌دامنه و ۲٬۶۲۹ مکان ترکیبی‌اند. پرتکرارترین ترکیب‌ها:
...
(۸۲)، `cafe+fastfood` (۷۳)، `tabbakh+fastfood` (۵۹).
...
چهارم، کترینگ‌ها (`craft=caterer`) زیرمجموعهٔ رستوران‌اند. پنجم، **نقشهٔ فست‌فود
با قاعدهٔ نام‌محور ۳۷۸ مکان رشد کرد** و نقشهٔ رستوران ۲۷۹ مکان را از دست داد؛
چون OSM در ایران بسیاری از ساندویچی/پیتزایی/برگری را `amenity=restaurant`
ثبت می‌کند و رستوران نیستند (پایین را ببین). ششم، ۸ مکان فست‌فود هیچ
...
- الگوهای **نام‌محور** فست‌فود (`*ساندویچ*`/`*فلافل*`/`*پیتزا*`/`*برگر*`/
  `*هاتداگ*`/`*فست*`/`*شاورما*` و معادل‌های لاتین) هم در فیلتر لازم‌اند: بعضی
...
- **خودآزمون پیش از استخراج:** `python extract-cafes.py --self-test` جدول
  قراردادِ ۱۰۸ موردی طبقه‌بندی را می‌سنجد و با هر اختلاف، کد خطا برمی‌گرداند.
...
# مرحله ۱: فیلتر سریع
# توجه: این فهرست «نمونهٔ کوتاه» است. مرجع رسمی و کامل الگوها (۲۹۴ الگو، شامل
...
| `fastfood` | `amenity=fast_food` **یا** (نام غذاخوری شامل کلیدواژهٔ فست‌فود) | `burger` |
...
- فست‌فود (مرزکلمه‌ای): `دنر` (املای بدون واو، مثل «دنر ترک»).
...
دادهٔ ناخالص می‌سازد؛ پس وقتی سیگنال نام فست‌فود وجود دارد، دامنهٔ `restaurant`
**برداشته** می‌شود و مکان فقط روی نقشهٔ فست‌فود (به‌همراه کباب/طباخی/کافه در
صورت وجود سیگنال) می‌آید. دو استثنا دارد: (۱) مکان **کترینگ** (تگ یا نام
کترینگ/پذیرایی) زیرمجموعهٔ رستوران است و می‌ماند؛ (۲) مکانی که نامش صریحاً
واژهٔ «رستوران» (یا `restaurant`) را دارد — مثل «رستوران و فست فود پرک» —
کسب‌وکار **ترکیبی** است و روی هر دو نقشه می‌ماند. این قاعده به دامنهٔ
`cafe`/`juice_icecream`/`kebab`/`tabbakh` دست نمی‌زند.

عدد واقعی این تمیزکاری: **۲۷۹ مکان** از نقشهٔ رستوران برداشته شد و نقشهٔ
فست‌فود از ۳٬۸۶۲ به **۴٬۲۴۰** رسید (+۳۷۸)؛ بخشی از این رشد مکان‌هایی است که
کافه هم بودند و ۸ مکان تازه فقط با الگوهای نام‌محور **فیلتر کاندید** شناسایی
شدند. دامنهٔ کافه/آبمیوه/کباب/طباخی **دقیقاً بدون تغییر** ماند.
