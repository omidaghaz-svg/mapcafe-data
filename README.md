...
**آمار واقعی (اجرای کامل روی PBF ایران، ۱ مهر ۱۴۰۵):** کل خروجی ۱۸٬۹۴۰ مکان
(۱۷٬۱۱۰ گره + ۱٬۷۲۷ way + ۱۰۳ relation). شمار مکان‌های هر نقشه (مکان ترکیبی در
هر نقشه جداگانه شمرده می‌شود):

| نقشه | شمار مکان | گره | way | relation | نام‌دار |
| --- | --- | --- | --- | --- | --- |
| رستوران | ۸٬۹۸۳ | ۷٬۶۹۱ | ۱٬۲۳۵ | ۵۷ | ۸٬۳۱۲ |
| کافه | ۴٬۹۸۶ | ۴٬۶۸۵ | ۲۸۹ | ۱۲ | ۴٬۵۴۶ |
| فست‌فود | ۴٬۲۸۹ | ۴٬۰۶۲ | ۲۰۰ | ۲۷ | ۳٬۹۱۲ |
| آبمیوه و بستنی | ۱٬۲۶۵ | ۱٬۱۸۸ | ۶۹ | ۸ | ۱٬۱۶۰ |
| کباب | ۱٬۷۲۳ | ۱٬۵۵۱ | ۱۶۰ | ۱۲ | ۱٬۶۸۹ |
| طباخی | ۵۵۲ | ۵۲۱ | ۲۸ | ۳ | ۵۴۹ |

از این میان ۱۶٬۳۰۶ مکان تک‌دامنه و ۲٬۶۳۴ مکان ترکیبی‌اند. پرتکرارترین ترکیب‌ها:
`kebab+restaurant` (۱٬۲۳۱)، `tabbakh+restaurant` (۳۹۹)، `kebab+fastfood` (۲۵۹)،
`cafe+juice_icecream` (۱۶۱)، `cafe+restaurant` (۹۶)، `juice_icecream+fastfood`
(۸۳)، `cafe+fastfood` (۷۳)، `tabbakh+fastfood` (۵۸).

**شش نکتهٔ مهم آماری:** اول، دامنهٔ کافه **دقیقاً** همان ۴٬۹۸۶ مکان پیشین است
...
چهارم، کترینگ‌ها (`craft=caterer`) زیرمجموعهٔ رستوران‌اند. پنجم، **نقشهٔ
فست‌فود با قاعدهٔ نام‌محور و cuisine‌محور ۴۲۷ مکان رشد کرد** و نقشهٔ رستوران ۳۲۲
مکان را از دست داد؛ چون OSM در ایران بسیاری از ساندویچی/پیتزایی/برگری را
`amenity=restaurant` ثبت می‌کند و رستوران نیستند (پایین را ببین). ششم، ۸ مکان فست‌فود هیچ
`amenity`/`shop` غذایی نداشتند و فقط با `cuisine` غیراستاندارد و نامشان
شناخته شدند؛ برای همین **فیلتر کاندید** هم الگوهای نام‌محور فست‌فود گرفت
(پایین، بخش فیلتر).
...
- الگوهای **نام‌محور** فست‌فود (`*ساندویچ*`/`*فلافل*`/`*پیتزا*`/`*برگر*`/
  `*هاتداگ*`/`*فست*`/`*شاورما*`/`*سوخاری*`/`*وینگ*` و معادل‌های لاتین) هم در فیلتر لازم‌اند: بعضی
  ساندویچی/پیتزایی/فلافلی هیچ `amenity`/`shop` غذایی ندارند و فقط با `cuisine`
  غیراستاندارد (`regional`/`breakfast`) و نامشان ثبت شده‌اند. شاهد واقعی:
  «فلافل کاکو» با `cuisine=regional` و «پیتزا پارک» با `cuisine=pizza` که
  بدون این الگوها هرگز کاندید نمی‌شدند (۸ مکان).
- توجه: در `tags-filter` نوشتن **الگوی چندگانهٔ وایلدکارد با کاما** معتبر نیست
  (`cuisine=*pizza*,*burger*` هیچ چیزی نمی‌گیرد)؛ هر الگو باید جدا نوشته شود.
  کامای ساده فقط برای مقادیر دقیق درست است (`shop=ice_cream,juice`).
...
- **خودآزمون پیش از استخراج:** `python extract-cafes.py --self-test` جدول
  قراردادِ ۱۲۵ موردی طبقه‌بندی را می‌سنجد و با هر اختلاف، کد خطا برمی‌گرداند.
...
# مرحله ۱: فیلتر سریع
# توجه: این فهرست «نمونهٔ کوتاه» است. مرجع رسمی و کامل الگوها (۳۰۳ الگو، شامل
# دامنه‌های رستوران/فست‌فود/کباب/طباخی/کترینگ/آبمیوه و الگوهای نام‌محور فارسی)
# در `.github/workflows/extract-cafes.yml` است؛ برای تست دقیق همان دستور را
# از ورک‌فلو بردار.
...
| `fastfood` | `amenity=fast_food` **یا** (نام غذاخوری شامل کلیدواژهٔ فست‌فود) **یا** (`amenity=restaurant` و `cuisine` تماماً فست‌فود و نامِ نشسته‌خوری/رستوران نداشته باشد) | `burger` |
| `restaurant` | `amenity=restaurant` **یا** تگ کترینگ (`shop=catering`/`amenity=catering`/`craft=caterer`) **یا** (نام غذاخوری شامل کلیدواژهٔ کترینگ)؛ **منهای تمیزکاری نقشهٔ رستوران** (پایین را ببین) | `utensils` |
...
- فست‌فود (زیررشته‌ای): `ساندویچ`، `ساندویج`، `فلافل`، `شاورما`، `پیتزا`،
  `برگر`، `هاتداگ`، `فستفود` (شامل «فست فود» و «فست‌فود»)، `دونر`، `döner`،
  `سوخاری`، `wings`، `sandwich`، `falafel`، `shawarma`، `pizza`، `burger`،
  `hotdog`، `doner`.
- فست‌فود (مرزکلمه‌ای): `دنر` (املای بدون واو، مثل «دنر ترک»)، `وینگ`
  (تا با «وینگر» قاطی نشود).

**تمیزکاری نقشهٔ رستوران (قاعدهٔ مهم):** OSM در ایران بسیاری از
ساندویچی/پیتزایی/برگری/فلافلی/سوخاری‌فروشی و مغازه‌های «فست فود» را
`amenity=restaurant` ثبت کرده است. چنین مکانی رستوران نیست و روی نقشهٔ رستوران
دادهٔ ناخالص می‌سازد؛ پس وقتی **سیگنال فست‌فود** وجود دارد، دامنهٔ `restaurant`
**برداشته** می‌شود و مکان فقط روی نقشهٔ فست‌فود (به‌همراه کباب/طباخی/کافه در
صورت وجود سیگنال) می‌آید. «سیگنال فست‌فود» یعنی **یا** سیگنال نام‌محور
(پایین‌تر) **یا** سیگنال cuisine‌محور. سه استثنا دارد: (۱) مکان **کترینگ**
(تگ یا نام کترینگ/پذیرایی) زیرمجموعهٔ رستوران است و می‌ماند؛ (۲) مکانی که نامش
صریحاً واژهٔ «رستوران» (یا `restaurant`) را دارد — مثل «رستوران و فست فود پرک»
— کسب‌وکار **ترکیبی** است و روی هر دو نقشه می‌ماند؛ (۳) مکانی که نامش
«نشسته‌خوری» را اعلام می‌کند (`تالار`/`سفرهخانه`/`آشپزخانه`/`آشپزخونه`/
`پذیرایی`/`بریان`/`بریانکده`) رستوران می‌ماند. این قاعده به دامنهٔ
`cafe`/`juice_icecream`/`kebab`/`tabbakh` دست نمی‌زند.

**طبقه‌بندی cuisine‌محور فست‌فود (قاعدهٔ مکمل، فقط روی `amenity=restaurant`):**
بسیاری از برندهای زنجیره‌ای پیتزا/برگر نامشان هیچ کلیدواژهٔ فست‌فودی ندارد
(«عطاویچ»، «تینو»، «چارچیز»، «اژدر زاپاتا»، «مانستر گریل»)، ولی `cuisine`شان
**تماماً** از توکن‌های بی‌ابهام فست‌فود است. اگر `amenity=restaurant` باشد و
`cuisine` غیرخالی و همهٔ مقادیرش در `pizza`/`burger`/`sandwich`/`falafel`/
`hot_dog`/`hotdog` باشند و نامش «نشسته‌خوری» یا «رستوران» را اعلام نکند، مکان
فست‌فود است. شرط `amenity=restaurant` عمدی است: هدف رفع اشتباهِ ثبتِ OSM در همین
تگ است؛ **کافه‌ای که `cuisine=pizza` دارد کافه می‌ماند** و فست‌فود شمرده نمی‌شود.
دو استثنای عمدی در توکن‌ها: (۱) `chicken` نیست، چون در ایران بیشتر یعنی
جوجه/مرغ سنتی نشسته (اکبرجوجه، مرغ بریان)، نه سوخاری؛ (۲) `doner`/`shawarma`/
`kebab` نیستند، چون قلمروِ دامنهٔ کباب‌اند.

عدد واقعی این تمیزکاری (v14): **۳۲۲ مکان** از نقشهٔ رستوران برداشته شد و نقشهٔ
فست‌فود از ۳٬۸۶۲ به **۴٬۲۸۹** رسید (+۴۲۷، شامل +۳۷۸ از قاعدهٔ نام‌محور و +۴۹ از
قاعدهٔ cuisine‌محور)؛ بخشی از این رشد مکان‌هایی است که کافه/آبمیوه هم بودند و ۸
مکان تازه فقط با الگوهای نام‌محور **فیلتر کاندید** شناسایی شدند. دامنهٔ
کافه/آبمیوه/کباب/طباخی **دقیقاً بدون تغییر** ماند.

**گاردِ محصول نانوایی:** «نان ساندویچی» یعنی نانِ مخصوص ساندویچ (کالای
نانوایی)، نه مغازهٔ ساندویچی؛ پس نام‌های `نانساندویچ`/`نانساندویج`/`نانپیتزا`/
`نانبرگر`/`نانهمبرگر`/`نانهاتداگ` از دامنهٔ فست‌فود بیرون می‌مانند.
